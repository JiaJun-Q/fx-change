...
public class RateRepository {
    ...
    private static final RowMapper<Rate> MAPPER = (rs, n) -> new Rate(
            ...
            asLocalDateTime(rs.getTimestamp("captured_at")));

    private static java.time.LocalDateTime asLocalDateTime(Timestamp ts) {
    ...
    public List<Rate> findLatest() {
        return jdbc.query("""
            ...
            WHERE r.id = (SELECT r2.id FROM fx_rate r2
                          WHERE r2.base_code = r.base_code
                            AND r2.quote_code = r.quote_code
